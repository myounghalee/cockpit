...
# 프롬프트
prompt_header='다음은 Claude Code 한 세션의 대화 기록입니다. 사용자가 나중에 "이 세션에서 뭘 했지?" 라고 회고할 때 볼 한 단락 요약을 작성하세요.

판단 규칙 — 다음 중 하나라도 있으면 **의미 있는 세션** 으로 간주하고 요약하세요:
  • 구현·수정·커밋 등 실제 작업
  • 설계/방향/우선순위/비교 등 **결정이나 기획 논의** (도구·기술·제품 선택 포함)
  • 개념·사실·제약 등 **새로 알게 된 내용** (학습, 조사 결과)
  • 아이디어 정리·스펙 초안·트레이드오프 검토
  • 문제 진단·원인 분석

아래 경우에만 정확히 "NONE" 한 단어만 출력하세요:
  • 순수 인사·잡담 1~2턴
  • 답만 들으면 끝나는 단순 사실 질문(날씨·시각 같은 것)
